...
def test_sparse_complex_transient():
    ...
    # Convert to split real/imag representation to avoid Diffrax complex warnings
    y_op_split = jnp.concatenate([y_op.real, y_op.imag])
    ...
    sol = diffrax.diffeqsolve(term, solver, t0=0.0, t1=t_max, dt0=1e-11, y0=y_op_split, args=(groups, sys_size), saveat=saveat, max_steps=1000)
    
    assert sol.ys.shape == (20, 2 * sys_size)
    assert jnp.isfinite(sol.ys).all()
    
    # Reconstruct complex result for checking
    y_final_c = sol.ys[-1, :sys_size] + 1j * sol.ys[-1, sys_size:]
    assert jnp.abs(y_final_c[port_map["R1,p"]]) > 0.1
