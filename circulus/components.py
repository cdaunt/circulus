...
def VoltageSource(signals: Signals, s: States, t: float, V: float = 0.0, delay: float = 0.0):
    """Step voltage source."""
...
@source(ports=("p1", "p2"), states=("i_src",))
def SmoothPulse(signals: Signals, s: States, t: float, V: float = 1.0, delay: float = 1e-9, tr: float = 1e-10):
...
@source(ports=("p1", "p2"), states=("i_src",))
def VoltageSourceAC(signals: Signals, s: States, t: float, V: float = 0.0, freq: float = 1e6, phase: float = 0.0, delay: float = 0.0):
...
@component(ports=("p1", "p2"))
def Diode(signals: Signals, s: States, Is: float = 1e-12, n: float = 1.0, Vt: float = 25.85e-3):
...
@component(ports=("p1", "p2"))
def ZenerDiode(signals: Signals, s: States, Vz: float = 5.0, Is: float = 1e-12, n: float = 1.0, Vt: float = 25.85e-3):
    vd = signals.p1 - signals.p2
...
def _nmos_current(v_d, v_g, v_s, Kp, W, L, Vth, lam):
    ...
    return jnp.where(vgs <= Vth, 
                     0.0,
                     jnp.where(vds < v_over, linear_current, sat_current))

@component(ports=("d", "g", "s"))
def NMOS(signals: Signals, s: States, Kp: float = 2e-5, W: float = 10e-6, L: float = 1e-6, Vth: float = 1.0, lam: float = 0.0):
    i_ds = _nmos_current(signals.d, signals.g, signals.s, Kp, W, L, Vth, lam)
    return {"d": i_ds, "g": 0.0, "s": -i_ds}, {}

@component(ports=("d", "g", "s"))
def PMOS(signals: Signals, s: States, Kp: float = 1e-5, W: float = 20e-6, L: float = 1e-6, Vth: float = -1.0, lam: float = 0.0):
...
@component(ports=("d", "g", "s"))
def NMOSDynamic(signals: Signals, s: States, Kp: float = 2e-5, W: float = 10e-6, L: float = 1e-6, Vth: float = 1.0, lam: float = 0.0, Cox: float = 1e-3, Cgd_ov: float = 1e-15, Cgs_ov: float = 1e-15):
...
def _junction_charge(v, Cj0, Vj, m):
    ...
    q_normal = -Cj0 * Vj / (1.0 - m) * (1.0 - jnp.power(jnp.maximum(0.0, 1.0 - v/Vj), 1.0 - m) - 1.0)
...
def BJT_NPN(signals: Signals, s: States, Is: float = 1e-12, BetaF: float = 100.0, BetaR: float = 1.0, Vt: float = 25.85e-3):
...
@component(ports=("c", "b", "e"))
def BJT_NPN_Dynamic(signals: Signals, s: States, Is: float = 1e-12, BetaF: float = 100.0, BetaR: float = 1.0, Vt: float = 25.85e-3, Cje: float = 1e-12, Cjc: float = 1e-12, Vje: float = 0.75, Vjc: float = 0.75, Mje: float = 0.33, Mjc: float = 0.33, Tf: float = 0.0, Tr: float = 0.0):
    # 1. DC Currents
    vbe = signals.b - signals.e
    vbc = signals.b - signals.c
...
def VoltageControlledSwitch(signals: Signals, s: States, Ron: float = 1.0, Roff: float = 1e6, Vt: float = 0.0):
